...
public class PersonFacade implements Serializable {
	...
	public void updatePerson(Person person) {
		EntityManagerHelper.beginTransaction();
		Person persistedPerson = EntityManagerHelper.getEntityManager().merge(person);
		persistedPerson.setName(person.getName());
		persistedPerson.setBirthDate(person.getBirthDate());
		persistedPerson.setCity(person.getCity());
		EntityManagerHelper.commitAndCloseTransaction();
	}
	...
	public Person findPersonWithCity(int personId) {
		EntityManagerHelper.beginTransaction();
		Person person = personDAO.findPersonWithCity(personId);
		EntityManagerHelper.commitAndCloseTransaction();
		return person;
	}
