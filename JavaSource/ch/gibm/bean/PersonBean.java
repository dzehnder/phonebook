...
public class PersonBean extends AbstractBean implements Serializable {
	...
	public List<City> listAllCitiesWithFilter(String name) {
		//get all cities
		List<City> cityList = new ArrayList<City>(this.cityBean.getAllCities());
		
		//remove when city name not occurs
		cityList.removeIf(l -> l.getName().toLowerCase().contains(name.toLowerCase()) == false);
		return cityList;
	}
